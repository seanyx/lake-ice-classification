...
## project steps
1. subsample lakes from lake ice dataset then export to GEE to derive training dataset (0. Sample lake ice observations for building training dataset)
2. attach SR image id to each record (Prepare_lake_id_and_image_id.js)
2. manually draw polygons of various lake ice class in GEE (f91f43f44d796d8f2b8c65c50b774f55w650p2)
3. subsample the pixels in the polygons and download the data to R
4. build a classification tree model in R (this script) and validation with the manually selected data, and also compare it with Fmask
5. apply the classifier in GEE to lakes having in situ lake ice data, export results, evaluate performance v.s. Fmask and in situ records.
...
## check lake name across datasets
hl_glrip = st_read(dsn = "outputs/HydroLAKES_glrip_lakes.shp")
hl_glrip %>% 
  st_drop_geometry() %>% 
  select(Hylak_id, Lake_name, glrip_id) %>% 
  distinct() %>% 
  left_join(glrip %>% select(glrip_id = id, lakename) %>% distinct(), by = "glrip_id") %>% 
  # as_tibble %>% 
  print(n.max = 100)
```
...
val = read_csv("data/lake_ice_fraction_glrip_db018e78a618b03f16df5d604ac4e4b5.csv")

version = "db018e78a618b03f16df5d604ac4e4b5"

load("outputs/GLRIP_cleaned.RData", verbose = T)

val = val %>% na.omit()

## map lakes in the validation
# glrip
...
p1 = val %>% 
  ...
  geom_point(aes(color = as.integer(cloud_mean * 100)), pch = 1, alpha = 0.8) +
...
save(merged_val, file = paste0("outputs/merged_val_", version, ".RData"))

## map of lake locations used available for validation
merged_val %>% filter(cloud_mean <= 0.1) %>% nrow()
merged_val %>% filter(cloud_mean <= 0.1) %>% select(id) %>% distinct
...
map = lakes %>% 
  ...
  ggplot() +
  geom_sf(data = world, fill = "black", color = NA) +
  geom_sf(data = glrip %>% select(id, longitude, latitude) %>% distinct %>% st_as_sf(coords = c("longitude", "latitude"), crs = 4326), aes(color = "Lakes with\nin situ data"), pch = 1, size = 0.1, show.legend = F) +
  geom_sf(aes(size = n), pch = 1, alpha = 0.5, color = "cyan") +
...
map %>% ggsave(filename = paste("figs/in_situ_data_map_", version, ".png"), width = 8, height = 4)


## validation stats
merged_val %>% 
  filter(cloud_mean <= 0.1) %>% 
  gather(key = "method", value = "ice_fraction", c(fmaskIce_mean, cartIce_mean)) %>% 
  mutate(dif = ice_fraction - `Ice cover`) %>% 
  group_by(method) %>% 
  summarise(
    mbs = mean(dif),
...
merged_val %>% 
  filter(cloud_mean <= 0.1) %>% 
  gather(key = "method", value = "ice_fraction", c(fmaskIce_mean, cartIce_mean)) %>% 
  mutate(correct = (ice_fraction >= 0.8 & `Ice cover` == T) | (ice_fraction <= 0.2 & `Ice cover` == F)) %>% 
  group_by(method, `Ice cover`) %>% 
  summarise(
...
validation_in_situ_transition_period = merged_val %>% 
  filter(cloud_mean <= 0.1) %>% 
...
validation_in_situ_transition_period

validation_in_situ_transition_period %>% 
  ggsave(filename = paste0("figs/validation_in_situ_transition_period_", version, ".png"),
         width = 8,
         height = 5)
...
inspection = merged_val %>% 
  ...
  dplyr::select(Hylak_id, LANDSAT_ID, cartIce_mean, fmaskIce_mean, glrip_id, date, `Ice cover`) %>% 
  arrange(desc(date))
```


## test reticulate visualize lakes
...
## image collections
ls5 = ee$ImageCollection("LANDSAT/LT05/C01/T1_SR")$select(c("B3", "B2", "B1"), c("Red", "Green", "Blue"))
...
for (i in 1:nrow(inspection)) {
  ...
  if (lsFil$size()$getInfo() == 1) {
    ...
    png(filename = paste0("figs/pngs/",thisRecord$LANDSAT_ID, "_", thisRecord$Hylak_id, ".png"), width = 800, height = 800, bg = "black")
