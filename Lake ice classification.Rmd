...
dat_wide_sr = (dat_wide %>% 
  mutate(class_int = as.integer(class)) %>% 
  filter(source == "SR"))[, c(1, 11, 13:24)]

require(rpart)
require(rpart.plot)

set.seed(2020)
sr_rpart = rpart(class_int~., data = dat_wide_sr %>% select(-fmask), method = "class")
...
validation = dat_wide_sr %>% bind_cols(pred = pred)

validation %>%
  # bind_cols(dat_wide %>% select(fmask)) %>% 
  ggplot() +
  geom_bar(aes(fmask, fill = pred)) +
...
dat_wide_sr = (dat_wide %>% 
  mutate(class_int = as.integer(class)) %>% 
  filter(source == "SR"))[, c(1, 5, 8, 11, 13:23)]
...
save(validation, file = "outputs/validation.RData")
